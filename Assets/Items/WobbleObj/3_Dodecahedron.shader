Shader "Item/Wobble/3_Dodecahedron"
{
    Properties {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			void geo(triangle appdata inp[3], inout TriangleStream<fin> strm) {
				...
				for(int i = 0; i < 3; i++) {
					pos[i] = inp[i].vertex;
					pos[i] = rotate(pos[i], radians(sin(_Time.y * 2) * 40), float3(0, 1, 0));
				}
				o.n = normalize(cross(pos[1] - pos[0], pos[2] - pos[0]));
				float3 cur = valueToMusic(_Value);
				float num = getVal(val3, cur)[0];
				float add = cur.z / 4 * 3;
				num = lerp(num, num+add, getVal(val3, cur)[1]);
				if (inp[0].uv.x != floor(num)) return;
				float3 center = float3(inp[1].uv.x, inp[1].uv.y, inp[2].uv.x);
				o.col = fixed4(rand3(inp[0].uv.x + 1, 395.2), 1);
				for (int i = 0; i < 3; i++) {
					o.pos = pos[i];
